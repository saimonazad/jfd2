...
Java2 1.4以上。ただし、ローカルディスクの容量、空き容量の表示を行うにはJava SE 6以上が必要です。
Macの場合はJava6以上。Java5以下の場合、ほとんどの場合正常に動作しますが、一部のファイル名を正しく扱うことができません（Appleにバグ報告済みです。対応されたらサポート対象に戻します）。
本バージョンはWindows2000、MacOSX10.4以降のみをサポート対象とします（他プラットフォームは所持していないので検証されていません。おそらく動きます）
...
JREのインストールが完了したら、適当なディレクトリにこのソフトのアーカイブを解凍し、jfd2.exeをダブルクリックしてください。なお、大きな画像を見る場合は、最大メモリ割り当てが大きい（128MB。通常は64MB）jfd128mb.exeを実行してください。
...
・Tiger以前のMac版Java5以下で、濁点、半濁点の文字が3つ以上連続するファイル名のファイルを作成すると間違った名称のファイルが作成されます。これはJava環境の実装のバグによるもので対処不能です。Java6を導入するか、Leopardに移行してください。
