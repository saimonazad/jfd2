...
public class ConfigVersionManager {
	...
	private static final FileVersion[] ETC_FILES_XML_WIN = {
		...
		new FileVersion("keys.xml", "keys.xml", 20090127),
	...
	public void checkVersion(VFile configDir) throws VFSException, JDOMException, IOException {
	...
	private void checkCommonConfigVersion(VFile configDir, VFile sourceDir)
		...
		if(editorPath == null || editorPath.length() == 0) {
			FileInputPanel panel = 
				new FileInputPanel(DefaultConfig
					.getDefaultConfig().getEditor(), JFDResource.MESSAGES
					.getString("input_editor_path"),
					JFileChooser.FILES_AND_DIRECTORIES);
			JOptionPane.showConfirmDialog(null, panel, "jFD2", JOptionPane.OK_OPTION);
	...
	private void checkLocalConfigVersion(VFile configDir, VFile sourceDir)
		...
		if (!configFile.exists()) {
			sourceDir.getChild(LOCAL_CONFIG_FILE).copyTo(configDir.getChild(LOCAL_CONFIG_FILE));
		}
	}
	
	private void checkEtcFilesVersion(VFile configDir, VFile sourceDir) throws VFSException {
		List toUpdate = new ArrayList();
		
		FileVersion[] fileVersions = isWindows ? ETC_FILES_XML_WIN : ETC_FILES_XML_UNIX;
		
		for(int i=0; i<fileVersions.length; i++) {
			VFile configFile = configDir.getChild(fileVersions[i].getFileName());
			if(configFile.exists()) {
				...
				sourceDir.getChild(fileVersions[i].getSourceFileName()).copyTo(configFile);
			}
		}
		...
		String message = MessageFormat.format(JFDResource.MESSAGES.getString("update_config_files"), values);
		int answer = JOptionPane.showConfirmDialog(null, message, "jFD2", JOptionPane.YES_NO_OPTION);
		if(answer == JOptionPane.NO_OPTION) {
			return;
		}
		
		for(int i=0; i<toUpdate.size(); i++) {
			VFile toFile = configDir.getChild( ((FileVersion)toUpdate.get(i)).getFileName() );
			VFile sourceFile = sourceDir.getChild( ((FileVersion)toUpdate.get(i)).getSourceFileName() );
			toFile.copyTo(toFile.getParent().getChild(toFile.getName() + ".old"));
	...
	public static double getXmlVersion(VFile file) throws VFSException {
		...
		try {
			...
			String versionStr = doc.getRootElement()
	...
	private static class FileVersion {
		...
		public FileVersion(String fileName, String sourceFileName, double version) {
