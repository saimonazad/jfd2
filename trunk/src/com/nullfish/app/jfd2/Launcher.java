...
//			System.setProperty( "sun.java2d.ddscale", "true");
			...
			if(configDirStr == null || configDirStr.length() == 0) {
				...
				try {
					realPath = VFS.getInstance().getFile(configDirStr).getAbsolutePath();
				} catch (Exception e) {}
			...
			try {
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(null, JFDResource.MESSAGES.getString("failed_to_install_config_file"));
				System.exit(1);
			}
			// プラグイン機能
			...
			VFile pluginDir = VFS.getInstance().getFile((String)commonConfig.getParam("plugin_dir", null));
	...
	public static void openJFD(CommandLineParameters params, VFile configDir) throws VFSException {
		...
		try {
			...
			FileViewerManager.getInstance().init(frame.getConfigDirectory());
		...
		try {
			...
			if((!firstDirRequested && !dir.getFileSystem().isLocal()) || !dir.exists()) {
				dir = VFS.getInstance(newJFD).getFile(System.getProperty("user.home"));
			}
			
			newJFD.getModel().setDirectoryAsynchIfNecessary(dir,
					dir.getParent(), newJFD);
		} catch (Exception e) {
			VFile dir = VFS.getInstance(newJFD).getFile(System.getProperty("user.home"));
			newJFD.getModel().setDirectoryAsynchIfNecessary(dir,
					dir.getParent(), newJFD);
		}
	...
	private static String getDefaultConfigDir() throws VFSException {
		VFile homeDir = VFS.getInstance().getFile(new File(System.getProperty("user.home")).getAbsolutePath());
		
		if(ConfigVersionManager.isWindows) {
			return homeDir.getRelativeFile("Application Data/Nullfish/jFD2/.jfd2").getAbsolutePath();
		} else {
